    ...
    const fetchTimeTrackingPreview = async () => {
      ...
      try {
        ...
        if (Array.isArray(fetchedSessions) && fetchedSessions.length > 0) {
          const days = fetchedSessions.map((s) =>
          ...
          setSessions(
            fetchedSessions
              .slice(0, 2)
              .map((s) => ({ name: s.name, duration: s.duration, tag: s.tag }))
          );
        } else {
          setStreak(0);
          setSessions([]);
      ...
      <div className="w-full flex flex-col gap-5 md:grid md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 md:gap-5">
        ...
        >
          <DashboardCard
            ...
            theme={theme}
          >
            <DashboardTaskPreview setTitle={setTasksCardTitle} />
          </DashboardCard>
        </div>
        <div
        ...
        <div
          ...
          style={{ cursor: "pointer" }}
          aria-label={t("goToTimeTrackingPage", {
            default: "Go to time tracking page",
          })}
        >
          <DashboardCard
            icon={<FaClock />}
            ...
            description={t("timeTrackingDescription", {
              default: "Track your work sessions and productivity streaks.",
            })}
            headerBg={
              theme === "dark"
                ? "bg-blue-900 border-blue-800"
