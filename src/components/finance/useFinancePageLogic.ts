...
export default function useFinancePageLogic() {
  ...
  const handleDelete = useCallback(
    (item: FinanceItem, type: "expense" | "income") => {
      setDeletedItem(item);
      setShowUndo(true);
  ...
  const finalizeDelete = useCallback(
    async (id: string, type: "expense" | "income") => {
      setLoading(true);
      try {
  ...
  const expensesData = useMemo(
    () => (statsRange === "week" ? expensesDataWeek : expensesDataMonth),
    [statsRange, expensesDataWeek, expensesDataMonth]
  );
  const incomesData = useMemo(
    () => (statsRange === "week" ? incomesDataWeek : incomesDataMonth),
    [statsRange, incomesDataWeek, incomesDataMonth]
  );

  const totalExpenses = useMemo(
    () => expensesData.reduce((total, amount) => total + amount, 0),
    [expensesData]
  );
  const totalIncomes = useMemo(
    () => incomesData.reduce((total, amount) => total + amount, 0),
    [incomesData]
  );
  ...
  const expenseTrend = useMemo(
    () =>
      expensesData[expensesData.length - 1] -
      (expensesData[expensesData.length - 2] || 0),
    [expensesData]
  );
  const incomeTrend = useMemo(
    () =>
      incomesData[incomesData.length - 1] -
      (incomesData[expensesData.length - 2] || 0),
  ...
  const chartLabels = useMemo(
    () => (statsRange === "week" ? weekLabels : monthLabels),
    [statsRange, weekLabels, monthLabels]
  );
  ...
  const expenseFormProps = useMemo(
    () => ({
      ...
      onDescriptionChange: (v: string) =>
        handleExpenseFormChange("description", v),
      onDateChange: (v: Date | null) =>
        handleExpenseFormChange("date", v ?? new Date()),
  ...
  const incomeFormProps = useMemo(
    () => ({
      ...
      onDescriptionChange: (v: string) =>
        handleIncomeFormChange("description", v),
