    ...
    <>
      ...
      <div className="flex flex-col gap-2 w-full sm:hidden">
        <div className="flex flex-col items-center min-w-[90px] px-2 py-1">
          <span className="text-[11px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{t("expensesCard")}</span>
          <span className="text-2xl font-bold text-red-500">${totalExpenses.toFixed(2)}</span>
        </div>
        <div className="flex flex-col items-center min-w-[90px] px-2 py-1">
          <span className="text-[11px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{t("incomeCard")}</span>
          <span className="text-2xl font-bold text-green-500">${totalIncomes.toFixed(2)}</span>
        </div>
        <div className="flex flex-col items-center min-w-[90px] px-2 py-1">
          <span className="text-[11px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{t("profitCard")}</span>
          <span className={`text-2xl font-bold ${profit >= 0 ? 'text-green-500' : 'text-red-500'}`}>{profit >= 0 ? '+' : '-'}${Math.abs(profit).toFixed(2)}</span>
        </div>
      </div>
