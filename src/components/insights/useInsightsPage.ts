...
export function useInsightsPage() {
  ...
  const [expenses, setExpenses] = useState<{ amount: number; category?: string; type?: string; date?: string; createdAt?: string }[]>([]);
  const [incomes, setIncomes] = useState<{ amount: number; category?: string; type?: string; date?: string; createdAt?: string }[]>([]);
  const [timeSessions, setTimeSessions] = useState<{ duration: number; createdAt?: string }[]>([]);
  const [aiTab, setAiTab] = useState<'analytics' | 'suggestions'>('analytics');
  const [aiAnalytics, setAiAnalytics] = useState('');
  const [aiSuggestions, setAiSuggestions] = useState('');
  const [loading, setLoading] = useState(true);
  const [taskStats, setTaskStats] = useState({ completed: 0, total: 0, last7Days: [0,0,0,0,0,0,0] });
  const [financeStats, setFinanceStats] = useState({ expenses: [0,0,0,0,0,0,0], incomes: [0,0,0,0,0,0,0], labels: ['6d ago','5d ago','4d ago','3d ago','2d ago','Yesterday','Today'] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const now = new Date();
        const last7dTasks: { completed?: boolean; updatedAt?: string; createdAt?: string }[] = [];
        ...
        expensesData.forEach((e: { category?: string; amount: number }) => { pie[e.category || 'Other'] = (pie[e.category || 'Other'] || 0) + e.amount; });
        setPieData(pie);
        const incomePie: { [cat: string]: number } = {};
        incomesData.forEach((i: { category?: string; amount: number }) => { incomePie[i.category || 'Other'] = (incomePie[i.category || 'Other'] || 0) + i.amount; });
  ...
  const memoFinanceStats = useMemo(() => {
    ...
    const now = new Date();
    expenses.forEach((e) => {
      const daysAgo = Math.floor((now.getTime() - new Date(e.date ?? e.createdAt ?? 0).getTime()) / (1000*60*60*24));
      if (daysAgo >= 0 && daysAgo < 7) expensesByDay[6-daysAgo] += e.amount;
    });
    incomes.forEach((i) => {
      const daysAgo = Math.floor((now.getTime() - new Date(i.date ?? i.createdAt ?? 0).getTime()) / (1000*60*60*24));
      if (daysAgo >= 0 && daysAgo < 7) incomesByDay[6-daysAgo] += i.amount;
    });
